...
import type { Item, ItemOccurrence, TimeBlock } from '../../db/schema';
...
describe('assembleWindow', () => {
  it('emits one virtual view per expanded slot, each with a null id', () => {
    ...
    const views = assembleWindow([context(recurrent)], new Map(), new Map(), new Map(), from, to);
  ...
  it('skips a non-recurrent item that has no block (backlog stays out of the feed)', () => {
    ...
    const views = assembleWindow([context(task)], new Map(), new Map(), new Map(), from, to);
